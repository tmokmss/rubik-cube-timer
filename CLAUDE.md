# キューブ区間タイマー

ルービックキューブ(CFOP)の区間タイムを測る個人用Webアプリ。GitHub Pagesで静的ホストする。

- 公開先: https://tmokmss.github.io/rubik-cube-timer/
- Svelte 5 (runes) + Vite + TypeScript。`main` に push すると GitHub Actions がビルドして Pages に出す

## 構成の考え方

ロジックは `src/lib/core/` に UI 非依存の素の TS として置き、vitest で固める。
Svelte コンポーネントは表示と入力だけを持つ。フレームワークを替えることになっても
`core/` はそのまま持っていける。

```
src/
  main.ts                エントリ
  App.svelte             配線。キー入力とフォーカスの面倒はここで見る
  app.css                全部グローバル(理由は下記)
  lib/
    state.svelte.ts      記録・モード・目標の保持と localStorage への書き戻し
    timer.svelte.ts      計測の状態機械
    wakelock.svelte.ts   練習中に画面を消させない
    pwa.svelte.ts        Service Worker の更新通知
    drive.svelte.ts      Google Drive 同期(認証と進行)
    core/                UI 非依存。ここだけテストがある
      types.ts           型、区間の定義、目標の比例配分
      format.ts          時間の表示
      stats.ts           ao5 / ao12 / 区間平均
      storage.ts         localStorage の読み書き
      keys.ts            Space をどこで拾うかの判定
      sync.ts            端末間マージと墓標(純関数)
      drive.ts           Drive REST の呼び出し
    components/          表示のみ
```

CSS をコンポーネントに閉じずグローバルに置いているのは、区間の色を
`[data-stage="F2L"] { --c: var(--green) }` のように CSS 変数で親から子へ渡しているため。
Svelte のスコープ付き CSS だとこの継承が切れる。

## 操作仕様

- Space(または画面タップ)を押して離すとスタート。keydownで `armed`、keyupで計測開始
- 計測中はSpace / 画面のどこでもタップで次の区間へ。最後の区間で押すとストップして自動保存
- Escで計測中止(保存しない)。文字入力中のEscは入力欄から抜けてタイマーに戻る
- キーリピート(`e.repeat`)は無視。ストップ時のkeyupで次の計測が始まらないこと(`armed` はidle時のkeydownでしか立たない)
- モード: `4` = Cross/F2L/OLL/PLL、`3` = Cross/F2L/LL、`1` = 合計のみ。計測中は切り替え不可

### Space の効き(フォーカス)

**ここは何度か壊れているので触るときは注意。** 守りたい性質は1つだけ:

> 文字入力中だけ Space を譲る。それ以外では、どこにフォーカスがあっても必ずタイマーが取る。

そのために `App.svelte` で:

1. キーは `window` で拾い、`isTextEntry(e.target)` のときだけ何もせず返す
   (`keys.ts`。`readonly` の textarea も文字入力扱い)
2. それ以外は `preventDefault()` してから `restoreFocus(padEl)` でフォーカスをタイマーに戻す。
   これをしないと、押した直後のボタンにフォーカスが残り、Space がそのボタンの click も撃つ
3. ボタンや summary を押した後も `onclick` でフォーカスをタイマーに戻す
4. 戻すときは `focus({ preventScroll: true })`。画面下の「削除」を押した直後にページ先頭へ飛ばさない
5. 計測中は透明な膜(`.tap-guard`)で画面全体を覆う。画面のどこを叩いても区間が刻まれ、
   下のボタンには届かない

タイマーパッドの `tabindex` は `-1`。Tab順には入れず、プログラムからのフォーカス置き場としてだけ使う。

## PWA

ホーム画面に置いて全画面で使えるようにしてある。オフラインでも起動する
(本体はプリキャッシュ、Google Fonts は実行時キャッシュ)。

アイコンは `public/` の PNG。`scripts` は用意していないので、作り直すときは
元の `favicon.svg` と同じ配色(Cross青 / F2L緑 / OLL黄 / PLL赤)で書き出す。
透過ではなく全面塗りにしてある(角が白く残る事故を避けるため)。

### 更新の扱い(触るときは注意)

`registerType` は **`prompt`**。自動更新にすると新版の配信時にページがリロードされ、
**計測中だとその記録が飛ぶ**。なので通知バーを出して、反映のタイミングは利用者に委ねる。
通知バーの「更新する」は計測中は押せない。

workbox の `clientsClaim: true` は必須。`prompt` は `skipWaiting: false` なので、
これが無いと新しい Service Worker を有効化しても既存ページの主導権を取らず、
`controlling` が発火せずリロードが起きない。つまり
**「更新する」を押しても無言で何も起こらなくなる**。
`pwa.svelte.ts` 側にも保険のリロードを置いてある。

## 画面スリープ防止

`wakelock.svelte.ts`。Space / タップのたびに取り直し、**無操作が5分続いたら自分から手放す**。
掴みっぱなしだと机に置いたままのときに電池を食い続けるため。
タブが隠れると OS 側で解除されるので、`visibilitychange` で取り直している。
対応していない環境や、電池残量が少なくて断られた場合は黙って諦める。

## データ

localStorage キー: `cube-split-timer:v1`

```json
{
  "mode": "4",
  "goalMs": 30000,
  "deleted": [{ "id": "old1", "sig": "1789800000000|58230", "at": "2026-09-19T10:00:00.000Z" }],
  "solves": [
    {
      "id": "lx3k9a",
      "at": "2026-09-19T10:00:00.000Z",
      "total": 58230,
      "splits": [{ "name": "Cross", "ms": 5200 }, { "name": "F2L", "ms": 31000 }]
    }
  ]
}
```

- 時間はすべてミリ秒の整数。`solves` は時系列順(古い順)
- `splits` が空配列の記録は合計のみ
- `scramble` は任意。**スクランブルは自分で崩すので、アプリは生成も表示もしない。**
  以前の記録にだけ残っているので、型としては読み書きできるまま残してある
  (表示だけ消して生成を続けると、一度も回していない手順が記録に残り続けるため両方やめた)
- **記録は各自の localStorage にしか無い。初期値は空で、アプリ側に記録を埋め込まない**
  (以前は過去のスプレッドシート分9件を初回に流し込んでいたが、他の人が開いても見えてしまうので外した)
- LLの値は、`LL` がなければ `OLL + PLL` で代用する(`stageMs`)
- `deleted` は消した記録の墓標。同期のときに相手から復活させないために要る(下記)
- 既定値を持つ項目の追加は後方互換なのでキーは据え置く(`goalMs` はこれで足した)。
  既存データが読めなくなる変更をするときだけ `v2` にして `storage.ts` に移行処理を書く
- **JSON / CSV の持ち出しと取り込みは作ったが消した**(`core/io.ts` と `DataPanel`)。
  端末間は Drive 同期で足りる。したがって記録の控えは
  **各端末の localStorage と、Drive の appDataFolder にある1ファイルだけ**になる

## Google Drive 同期

記録を利用者自身の Google Drive の **appDataFolder**(アプリ専用の隠しフォルダ)に
1ファイル置いて、端末間で揃える。サーバーは無い。

- スコープは `drive.appdata` のみ。**非センシティブ**に分類されていて、利用者の他の
  ファイルには一切触れない。個人利用なら OAuth 同意画面を「テスト」にして
  自分をテストユーザーに入れれば審査は要らない
- 認証は Google Identity Services の **token model**(`initTokenClient`)。
  ブラウザだけで完結し、クライアントシークレットは要らない
- **クライアント ID はリポジトリに入れてよい。** 秘密ではなく、Google Cloud 側の
  「承認済みの JavaScript 生成元」で守る値。生成元は
  `https://tmokmss.github.io` と `http://localhost:5173`
- GIS のスクリプトは**同期するときだけ**読む。最初から読むと初回表示と
  オフライン起動に外部スクリプトを巻き込むことになる
- アクセストークンは localStorage に置く(`cube-split-timer:drive-token`)。
  開き直しても期限内なら認可をやり直さずに済む(自動同期はこの期限内でだけ動く)。**以前はメモリだけにしていたが、
  このアプリでは割に合わないと判断して変えた**:
  - 漏れて困る範囲が `drive.appdata` の中、つまりこのアプリの隠しフォルダだけ
  - 有効期間は1時間で、切れれば自然に無効になる
  - 盗み出す足場が無い。`@html` も `innerHTML` も使っておらず描画は全部
    Svelte のエスケープを通る、実行時依存はゼロ、他人のコンテンツを表示する経路も無い
  - ビルド時にサプライチェーンを汚染されれば取られるが、メモリに置いていても
    `callback` をフックされて同じこと。置き場所で露出は変わらない

  **リフレッシュトークンは持てない。** token model は `access_type=offline` を送らないし、
  code model に替えても交換に `client_secret` が要る(PKCE を付けても
  `client_secret is missing.` で 400)。バックエンドが無い以上ここは動かせない

### ログインを何度も聞かれないために(触るときは注意)

トークンは1時間で切れるので、開き直すたびに `#getToken` を通る。
だから**画面を出さずに取り直せるか**が使い勝手をほぼ決める。守るものは2つ。

1. **`prompt: ''`**。GIS の中でこれは「認可 URL から `prompt` を落とす」に化ける
   (`''` は `undefined` でも truthy でもないので、既定の `select_account` も付かない)。
   Google 側の判断に委ねられ、条件が揃えば無言でトークンが返る。
   `'none'` にすると未同意のとき即エラーになるので、ここは空文字でよい
2. **`hint`(= `login_hint`)**。これが**本体**。Google に複数アカウントでログインしていると、
   `prompt` を落としただけでは「どのアカウントか」が決まらず、**毎回アカウント選択が出る**。
   初回の同期で `about.get` からメールアドレスを取って `cube-split-timer:drive-account` に
   覚え、次から `login_hint` として渡す

`about.get` は `drive.appdata` スコープで呼べるので、ヒントのためにスコープは増えない
(`openid email` を足す必要は無い。足すと再同意が要る)。

覚えたアカウントが使えなくなると詰むので、`callback` が `error` を返したときだけ
ヒントを捨てて選び直させる。ウィンドウを閉じただけ(`error_callback`)は意図的な取り消しなので捨てない。
**アカウントを変えたいときは、この localStorage キーを消す。**

なお、同意画面の公開ステータスが「テスト」だと付与が7日で失効して同意画面が出る。
`drive.appdata` は非センシティブなので、審査なしで「本番」に公開できる。

### 自動同期のきっかけ

一度「同期」を押した後は、以下で勝手に揃う(`driveSync`)。

| きっかけ | 呼ぶもの | 間引き |
| --- | --- | --- |
| 記録が増えた / 消した | `changed()` | 最後の変化から4秒 |
| 起動 / 画面に戻った / オンラインに復帰 | `resume()` | 前回の試行から60秒。未送信の変更があれば無視して即送る |
| 「同期」ボタン | `sync()` | 無し |

PWA は閉じても再読み込みされないので、「開いたとき」は `onMount` だけでなく
`visibilitychange` でも拾う。ここを落とすとスマホでほぼ発火しない。

### 自動同期は認可を取りに行かない(触るときは注意)

**`#run('auto')` は、期限内のアクセストークンを既に持っているときだけ進む。**
無ければ何もせず、`needsAuth` を立ててボタンに委ねる。

理由は GIS の作りにある。token model の `requestAccessToken` は
**`prompt: ''` でも必ずポップアップウィンドウを開く**。ID token 側と違って
隠し iframe で済ませる道が無い。同意済みで一瞬で閉じる場合でも、操作していないのに
Google のウィンドウが出ることに変わりはない。起動のたびにこれが出るのは論外で、
スマホの PWA では画面ごと持っていかれる。

(実測: ローカルで `CONSENTED_KEY` と `ACCOUNT_KEY` を入れてリロードしただけで、
操作なしに認可ウィンドウが別タブで開いた。ポップアップブロックも当てにならない。)

したがって **`#getToken` に `auto` で入らせない**。トークンは1時間もつので、
一度押せばその間の計測・削除・開き直しは全部自動で揃う。切れたらまた押す。
ここを「無言で取り直せるはず」と書き換えると、起動のたびに Google の画面が出る。

自動の失敗は**黙って飲む**。エラーの帯を勝手に出さない。`pending` を立てたまま
次の機会に回す。401 のときだけ `needsAuth` を立てて「押してください」と出す。

### マージの決まり(`core/sync.ts`)

- 記録は**和集合**。両方にしかないものは両方に入る。衝突という概念が無い
- 同一性は id、または「日時+合計」の署名。id が食い違う経路ができても二重にならない
- **削除は墓標(`deleted`)で伝える。** 和集合だけだと「消した」が伝わらず、片方で消しても
  相手から復活してしまう。ここを省略しない
- 墓標は1年で刈る。それより長く同期していない端末が繋がると、そこで消した記録は復活しうる
- Drive に置いたファイルが**在るのに読めない**ときは、空として扱わず**中断する**。
  空とみなすと、他の端末にしか無い記録を手元の中身で上書きしてしまう。
  `parseSyncDoc` が `null` を返したら諦めるのが正しい
- ファイルは1つのはずだが、2台の初回同期がぶつかると2つできうる。`findFiles` は
  `createdTime` 順で全部返し、`mergeAll` が全部混ぜて先頭に書き戻す。
  どれを正とするかが端末ごとにぶれないように順序を固定している
- `mode` と `goalMs` は同期しない。端末ごとの設定のままにしてある

### 書き込みの衝突

Drive のファイルは **last-writer-wins** で、条件付き更新の手段が無い。2台が同時に同期すると
後から書いた方が相手の分を落とす。そのため書く直前にもう一度 `files.version` を読み、
取得時から変わっていたらマージからやり直す(`#push`、3回まで)。
版の確認と PATCH の間の隙間までは消せないが、**各端末は自分の控えを持ったまま**なので、
すり抜けても次の同期で戻る。恒久的に消えるのは「一度も同期していない端末の localStorage を
消したとき」だけ。

## 集計

- ao5 / ao12: 直近N回から最速と最遅を1つずつ除いた平均
- 区間平均: 区間つき記録の直近12回
- 目標は 60 / 45 / 30 秒から選ぶ。区間ごとの目安は30秒基準(Cross 4 / F2L 16 / OLL 5 / PLL 5)を
  比例配分する。`Cross + F2L + OLL + PLL` は必ず目標と一致する
- 推移グラフは2種類ある(パネル右上で切り替え)
  - **合計**: 1回ごとのタイムと ao5。対数軸、直近60回、選んだ目標に破線
  - **区間の内訳**: 区間ごとの積み上げ面グラフ(`stageTrend`)。こちらは線形軸
    (積み上げなので対数だと意味が壊れる)。帯の厚みがその区間の時間で、
    薄くなっていればその区間が縮んでいる。生の値は跳ねるので直近5回の移動平均にしている。
    指定した区間が全部そろっている記録だけを使う(欠けると積み上げの合計が狂うため)
  - 区間グラフの目標線は帯の上に重なるので、下地を敷いて色に埋もれないようにしてある

## デザイン

- 区間の色はキューブのステッカー色: Cross青 / F2L緑 / OLL黄 / PLL赤 / LLオレンジ
- 数字は Saira Semi Condensed(tabular-nums)、本文は Zen Kaku Gothic New
- ライト/ダーク両対応。色は `:root` のCSS変数で定義。ダークで反転する文言を書かない
  (「黒い線が目標」ではなく「縦線が目標」)

## 開発

```bash
pnpm install
pnpm dev      # http://localhost:5173/rubik-cube-timer/
pnpm test     # vitest (core のみ)
pnpm check    # svelte-check
pnpm build    # check してから vite build
pnpm preview
```

`base` は `/rubik-cube-timer/` 固定。dev でも同じパスにしてあるので、パス依存の取りこぼしに
ローカルで気づける。

## やりたいこと(未実装)

- 同期の対象に `goalMs` を入れる(最終更新時刻を持たせて後勝ちにする必要がある)
- セッション区切り(「今日の ao12」を全期間と分けて見る)
- インスペクション15秒のカウントダウン
- +2 / DNF
- 履歴の表示上限(今は新しい方から200件で打ち止め)
- 目標の自由入力(今は 60 / 45 / 30 の3択)
- 区間目標の配分を比例ではなく実際の分布に寄せる(速くなるほど F2L の比率は下がる)

直前の記録を1タップで取り消す案は見送った。画面全体がタップ領域なので、
取り消しボタン自体を誤爆する危険の方が大きいため。削除は履歴側から行う。
